...
var wintypesInit = function() {	
	...
	*/
	this.PROPVARIANT = new ctypes.StructType('PROPVARIANT', [
		...
		{ 'ulVal': this.ULONG } // for PKEY_AppUserModel_StartPinOption
	]);
...
function InitPropVariantFromBoolean(fVal/*jsbool*/, ppropvar/*ostypes.PROPVARIANT.ptr*/) {
	...
	ppropvar.contents.vt = ostypes.VT_BOOL;
	ppropvar.contents.boolVal = fVal ? ostypes.VARIANT_TRUE : ostypes.VARIANT_FALSE;
	return ostypes.S_OK;
}

/* https://msdn.microsoft.com/en-us/library/windows/desktop/bb762311%28v=vs.85%29.aspx
 * NOTE1: I have to write my own InitPropVariantFromUInt32 because its not in a dll its defined in a header of Propvarutil.h
 * I looked at https://github.com/jingoro2112/dna/blob/6f0f141abbd52225375b76db0ff28b141ed16ac1/dnausb/win32/ddk/api/propvarutil.h#L385
 * HRESULT InitPropVariantFromUInt32(
 *   __in_   ULONG ulVal,
 *   __out_  PROPVARIANT *ppropvar
 * );
 */
function InitPropVariantFromUInt32(jsInt_ulVal /*jsint*/, ppropvar/*ostypes.PROPVARIANT.ptr*/) {
	// returns ostypes.HRESULT
	var ulVal = ostypes.ULONG(jsInt_ulVal);
	ppropvar.contents.vt = ostypes.VT_UI4;
	ppropvar.contents.ulVal = ulVal;
	return ostypes.S_OK;
}
...
function IPropertyStore_SetValue_VT_UI4(vtblPpsPtr, pps/*IPropertyStore*/, pkey/*ostypes.REFPROPERTYKEY*/, pszValue/*ostypes.PCWSTR*/) { // i introduced vtblPpsPtr as i need it for js-ctypes
	// returns hr of SetValue, but if hr of it failed it will throw, so i dont have to check the return value
	
	var ppropvar = ostypes.PROPVARIANT();

	var hr_InitPropVariantFromUInt32 = InitPropVariantFromUInt32(pszValue, ppropvar.address());
	checkHRESULT(hr_InitPropVariantFromUInt32, 'failed InitPropVariantFromUInt32'); //this will throw if HRESULT is bad
...
function IPropertyStore_GetValue_VT_UI4(vtblPpsPtr, pps/*IPropertyStore*/, pkey/*ostypes.REFPROPERTYKEY*/, ppropvar /*ostypes.PROPVARIANT*/ /* OR NULL if you want jsstr returned */) {
	// currently setup for String propvariants only, meaning  key pwszVal is populated
	// returns hr of GetValue if a ostypes.PROPVARIANT() is supplied as ppropvar arg
	// returns jsstr if ppropvar arg is not supplied (creates a temp propvariant and clears it for function use)
	...
	if (ret_js) {
		console.info('ppropvar.ulVal:', ppropvar.ulVal, ppropvar.ulVal.toString(), uneval(ppropvar.ulVal));
		/* // i dont think uiVal has isNull func as its not a pointer
		if (ppropvar.ulVal.isNull()) {
			console.log('ppropvar.ulVal is NULL');
			var jsstr = '';
		} else {
			var jsstr = ppropvar.ulVal.readString();
		}
		*/
		var jsint = parseInt(ppropvar.ulVal); // weird even if i set this to `ppropvar.ulVal` and then do PropVariantClear and then return jsint, it returns without clearning, but to be safe im going with parseInt as I know its either 0 or 1 or 2
		// have to propvariant clear here too, as per msdn usage example
		var rez_PropVariantClear = _dec('PropVariantClear')(ppropvar.address());
		console.info('rez_PropVariantClear:', rez_PropVariantClear, rez_PropVariantClear.toString(), uneval(rez_PropVariantClear));
...
function main() {
	...
	try {
		...
		// start get PKEY's
		var fmtid_ID = fmtid_RelaunchCommand = fmtid_RelaunchDisplayNameResource = fmtid_RelaunchIconResource = fmtid_StartPinOption = new ostypes.GUID();
		...
		var PKEY_AppUserModel_StartPinOption = ostypes.PROPERTYKEY(fmtid_StartPinOption, 12);

		// end get PKEY's
		///*
		var hr_IPSSetValue = IPropertyStore_SetValue_VT_UI4(ppsPtr, pps, PKEY_AppUserModel_StartPinOption.address(), 1);
		...
		*/
		...
		var jsstr_IPSGetValue = IPropertyStore_GetValue_VT_UI4(ppsPtr, pps, PKEY_AppUserModel_StartPinOption.address(), null);
