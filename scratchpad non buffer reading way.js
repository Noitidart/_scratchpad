...
function enumHandles() {
    ...
    for (var i=0; i<proc.NumberOfHandles; i++) {
        ...
        //var pHandlesI = ctypes.cast(buffer.addressOfElement(i), SYSTEM_HANDLE_TABLE_ENTRY_INFO.ptr).contents;
      if (res[proc.Handles[i].UniqueProcessId]) {
        res[proc.Handles[i].UniqueProcessId].push(proc.Handles[i].HandleValue);
          res[proc.Handles[i].UniqueProcessId].push(proc.Handles[i].HandleValue);
        //console.error('multiple handlevalue occourance for handle value of ', res[proc.Handles[i].HandleValue])
      } else {
        res[proc.Handles[i].UniqueProcessId] = [proc.Handles[i].HandleValue];
      }
    }
    return res;
}
