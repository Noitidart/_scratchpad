...
function main() {
	...
	var shortcutFile = OS.Path.join(OS.Constants.Path.desktopDir, 'Mozilla Firefox - Dev.lnk'); // string path, must end in .lnk
	...
	promise_checkExists.then(
      ...
      var pszIconPath = ostypes.WCHAR.array(OS.Constants.Win.MAX_PATH)();
      var piIcon = ostypes.INT();
      var hr_GetIconLocation = shellLink.GetIconLocation(shellLinkPtr, pszIconPath, pszIconPath.length, piIcon.address());
      console.info('pszIconPath:', pszIconPath, pszIconPath.readString());
      console.info('piIcon:', piIcon);
