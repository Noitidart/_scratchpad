...
function duplicateDirAndContents(sourceDir, targetDir, max_depth, targetDirExists) {
	// returns promise
	...
	var promise_enumEntries = enumChildEntries(pathToTarget, delegate_handleEntry, max_depth, null, !targetDirExists);
	return promise_enumEntries.then(
	  function(aVal) {
...
var promise_duplicateDirAndContents = duplicateDirAndContents(pathToTarget, pathToCreate, null, false);
promise_duplicateDirAndContents.then(
	...
	function(aReason) {
		var refObj = {name:'promise_duplicateDirAndContents', aReason:aReason};
		console.error('Rejected - promise_duplicateDirAndContents - ', refObj);
		//throw refObj; // //dont throw here as its final
	}
).catch(
	function(aCaught) {
