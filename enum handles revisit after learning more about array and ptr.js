...
var SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX = ctypes.StructType('SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX', [ //typedef struct _TagHANDLEINFO
...
var SYSTEM_HANDLE_INFORMATION_EX = ctypes.StructType('SYSTEM_HANDLE_INFORMATION_EX', [
    {'NumberOfHandles': ctypes.unsigned_long},
    {'Reserved': ctypes.unsigned_long},
    {'Handles': ctypes.voidptr_t}
]);
...
function enumHandles() {
    ...
    //return {};
    var res = {};
    //console.log('size inv:', SYSTEM_HANDLE_INFORMATION_EX.size, SYSTEM_HANDLE_INFORMATION_EX.fields[2].Handles.size, SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX.size);
    
    var system_handle_info_ex = SYSTEM_HANDLE_INFORMATION_EX(); //ctypes.char.array(_enumBufSize.value)();
    var list = SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX.array(1)();
    system_handle_info_ex.Handles = list.address();
    console.error('list', list.length, SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX)
    //console.error('system_handle_info_ex.Handles', system_handle_info_ex.Handles.);
    //console.error('system_handle_info_ex.Handles.length', system_handle_info_ex.Handles.constructor.length, system_handle_info_ex.Handles.contents.constructor);
    //console.info('my', SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX.array(10)().constructor.length)
    //return res;
    var _enumBufSize = new ctypes.unsigned_long(SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX.size * list.length); //size when 1 element == 32. when 2 element == 60, 3 == 88// this is 32 - 4 / 4 == 7 fields. 60-4/4 == 14 fields
    console.info('_enumBufSize:', _enumBufSize.toString());
    ...
    var parsedNum2 = parseInt(system_handle_info_ex.NumberOfHandles);
    var useNum = Math.min(parsedNum, parsedNum2);
    if (parsedNum > parsedNum2) {
        console.log('there are less handles obtained on 2nd call, so will use less as max elements to go through');
    }
    ///end rep
...
var pidsToCollectHandlesFor = {
    '6220': 0, //pid of firefox that you are not running this code from
    '7880': 0,
    '8780': 0
}
