cDump(window,{t:'window'});

function cDump(obj, opts, ppOpts) { //ppOpts is prettyPrintOptions
    /*
    /////////////////
    KEYS AVAIL IN OPTS
    ...
    */
    ...
    if (!ppOpts.maxDepth) ppOpts.maxDepth = 1;
    ppOpts.cdOpts = opts; //cDumOpts, so i can put title in the thing
    
    var table = prettyPrint(obj, ppOpts);
    ...
    var onloadFunc = function() {
        ...
        if (opts.t) { newTabBrowser.contentDocument.title = opts.t; }
        newTabBrowser.contentDocument.body.appendChild(table);
    };
...
//https://github.com/padolsey/prettyPrint.js/tree/master
var prettyPrint = (function(){
	...
	var util = {
		...
		el: function(type, attrs) {
			...
			/* Create new element */
			var el = Services.wm.getMostRecentWindow(null).document.createElementNS('http://www.w3.org/1999/xhtml', type), attr;
		...
		txt: function(t) {
			/* Create text node */
			return Services.wm.getMostRecentWindow(null).document.createTextNode(t);
		},
		...
		type: function(v){
			try {
				...
				if (v === Services.wm.getMostRecentWindow(null).document.defaultView || v === Services.wm.getMostRecentWindow(null).document) {
					return 'object';
				}
				return 'default';
			} catch(e) {
		...
		headerGradient: (function(){
			return ''; //noit: fix this to css gradient someday
			var canvas = Services.wm.getMostRecentWindow(null).document.createElement('canvas');
