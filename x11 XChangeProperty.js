...
function main() {
	/*
	var myXData = ostypes.UNSIGNED_CHAR.ptr.array([
		ostypes.UNSIGNED_CHAR.array()('rawr')
	]);
	*/
	//var myXData = ostypes.UNSIGNED_CHAR.array()('rawr');
	//console.info('myXData:', myXData, myXData.toString(), uneval(myXData));
	//console.info('myXData:', myXData.address(), myXData.address().toString(), uneval(myXData.address()));
	var myXData = ostypes.UNSIGNED_CHAR.array()('rawr');
	console.info('myXData:', myXData, myXData.toString(), uneval(myXData));
	var rez_XChangeProp = _dec('XChangeProperty')(GetXDisplay(), xidFromXULWin(Services.wm.getMostRecentWindow('navigator:browser')), GetAtom('_NET_WM_NAME'), GetAtom('UTF8_STRING'), 8, ostypes.PROPMODEREPLACE, myXData, myXData.length);
